...
//Auswahl der Klasse:
$select_kla = $db->query("SELECT DISTINCT klasse FROM edoo_schueler_klasse WHERE schuljahr LIKE '$schuljahr' ORDER BY klasse ASC");
...
//$klasse = "BS STH 24A";
		...
		$select_an = $db->query("SELECT DISTINCT id_edoo, nachname, vorname, klasse, geburtsdatum, eintritt, beruf FROM edoo_schueler WHERE klasse LIKE '$klasse' ORDER BY {$sort} {$rfg}");
