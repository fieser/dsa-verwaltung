...
echo "<td style='".$style_kopf."' align='left'><a href='./liste.php?sort=geburtsdat.&rfg=".$rfg."'><b><nobr>Geburtsdatum</nobr></b></a></td>";
...
GROUP BY dsa_bewerberdaten.id 
ORDER BY dok_neu DESC,{$sort} {$rfg}
");
...
foreach($select_an as $an) {
	...
	*/
	...
	if ($an['dok_neu'] != 1 AND ($an['beginn'] == "" OR strtotime($an['beginn']) >= strtotime(date("Y",time())."-08-01") OR $an['status'] == "übertragen")) {
	...
	if ($an['schulform'] != "bs") {
		...
		$select_edoo = $db->query("SELECT id FROM edoo_bewerber WHERE geburtsdatum = '$geburtsdatum' AND nachname = '$nachname' AND geburtsort = '$geburtsort'");	
		...
		if ($treffer_edoo > 0 AND $temp_status_voll == 1) {
			...
			//Ohne Austritt:
			$select_edoo = $db->query("SELECT id FROM edoo_schueler WHERE geburtsdatum = '$geburtsdatum' AND nachname = '$nachname'");	
			$treffer_edoo = $select_edoo->rowCount();
			...
			foreach($select_mail as $mail) {
				//echo "<tr>".$mail['highest_last_time']."</tr>";
					...
					if ($mail['highest_last_time'] < (time() - 24*3600*10)) {
						echo "<td style='padding: 10px; align='left'><img width='20px' src='./images/mail_2.svg'></td>";
					} else {
						echo "<td style='padding: 10px; align='left'><img width='20px' src='./images/mail_1.svg'></td>";
					}
