...
if ($_SESSION['sek'] == 1 OR $_SESSION['admin'] == 1 OR $_SESSION['ft'] == 1) { //wenn keine Lehrkraft
	if ($_SESSION['papierkorb'] != 1) {
		echo "<h1>Anmeldungen Bewerberinnen und Bewerber ".$periode."</h1>";
	} else {
	...
	*/
	...
	if ($an['schulform'] != "bs") {
		...
		if ($treffer_edoo > 0 AND $temp_status_voll == 1) {
			...
			//Ohne Austritt:
			$select_edoo = $db_www->query("SELECT id, klasse FROM edoo_schueler WHERE geburtsdatum = '$geburtsdatum' AND nachname = '$nachname'");	
			$treffer_edoo = $select_edoo->rowCount();
