...
foreach($select_an as $an) {
	...
	*/
	...
	if ($an['schulform'] != "bs") {
		...
		if ($an['schulform'] == "bgy" OR $an['schulform'] == "bvj" OR $an['schulform'] == "bf1" OR $an['schulform'] == "bfp" OR $an['schulform'] == "aph" OR $an['schulform'] == "hbf") {
			//Status bei vollständigen Unterlagen:
		...
		if ($treffer_edoo > 0 AND $vollstaendig == 1) {
					if ($db->exec("UPDATE `dsa_bewerberdaten`
									   SET
			...
			if ($treffer_edoo_a > 0 AND $vollstaendig == 1) {
						if ($db->exec("UPDATE `dsa_bewerberdaten`
										   SET
