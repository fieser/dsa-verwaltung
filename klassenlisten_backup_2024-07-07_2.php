...
//Auswahl der Klasse:
$select_kla = $db->query("SELECT DISTINCT klasse FROM edoo_schueler_klasse WHERE schuljahr LIKE '$schuljahr' AND (klasse LIKE '%$schuljahr_ss%' OR klasse LIKE '%+%') ORDER BY klasse ASC");
...
//$klasse = "BS STH 24A";
	...
	$select_an = $db->query("SELECT DISTINCT 
    es.id_edoo, 
    es.nachname, 
    es.vorname, 
    es.klasse, 
...
WHERE 
    esk.klasse LIKE '$klasse' AND esk.schuljahr NOT LIKE '%statistik%' ORDER BY {$sort} {$rfg}");

		
		
		$select_berufe = $db->query("SELECT DISTINCT beruf FROM edoo_schueler WHERE klasse LIKE '$klasse' ORDER BY {$sort} {$rfg}");
		foreach($select_berufe as $berufe) {
			...
			$beruf_anz = $ber['langform'];
			$beruf_anz_kurz = substr($ber['langform'],0,5);
			}
			
		
			//Weitere Anmeldungen des Berufes suchen:
			$select_an_weiter = $db->query("SELECT DISTINCT dsa_bewerberdaten.*, dsa_bildungsgang.* FROM dsa_bewerberdaten LEFT JOIN dsa_bildungsgang ON dsa_bildungsgang.md5 = dsa_bewerberdaten.md5 WHERE papierkorb NOT LIKE '1' AND beruf_anz LIKE '$beruf_anz_kurz%' GROUP BY dsa_bewerberdaten.id ORDER BY {$sort} {$rfg}");
			foreach($select_an_weiter as $anw) {
				echo $anw['beruf']."<br>";
				echo $anw['nachname']."<br>";
...
foreach($select_an as $an) {
	...
	*/
	...
	//von / bis
	if ($an['eintritt'] != "") {
	echo "<td style='padding: 10px;' align='left'>".date("d.m.y",strtotime($an['eintritt']))."</td>";
	} else {
	...
	//Beruf/Schwerpunkt
		...
		$beruf = str_replace("1027_","",$an['beruf']);
		...
		echo "<td style='padding: 10px;' align='left'>".$an['klassengruppe']."</td>";
