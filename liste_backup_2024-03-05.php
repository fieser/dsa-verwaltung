...
foreach($select_an as $an) {
	...
	*/
	...
	if ($sum_prio == "" AND $an['prio'] != "") {
		...
		echo "<td style='padding: 10px;' align='left'>".strtoupper($an['schulform'])." <small><font color='grey'>(".$sum_prio.")</font></small></td>";
	} else {
	...
	if ($an['schulform'] != "bs") {
		...
		if ($treffer_edoo > 0 AND $temp_status_voll == 1) {
			...
			//Ohne Austritt:
			$select_edoo = $db->query("SELECT id FROM edoo_schueler WHERE geburtsdatum = '$geburtsdatum' AND nachname = '$nachname' AND geburtsort = '$geburtsort' AND austritt < '2000-01-01'");	
			$treffer_edoo = $select_edoo->rowCount();
